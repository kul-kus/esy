...
module.exports = {
  ...
  getAndChangeDateKeys: function (pl) {
    let date_arr = ["modificationDate", "creationDate", "timestamp", "addedAt", "saved-at-timestamp",
      "deleted-changed-timestamp", "created", "updated",
      "updated_date_time", "date", "publish_date", "publish_date_local_time",
      "deleted_at", "created_time", "freeze_date", "published_at",
      "unpublished_at", "created_date_time", "published_at"]

    let del_key = ["macroRenderedOutput", "extensions", "_expandable"]

    if (pl) {
      Object.keys(pl).map((f) => {
        if (pl[f] != null) {
          if (Array.isArray(pl[f])) {
            pl[f] = pl[f].map((k) => {
              return module.exports.getAndChangeDateKeys(k)
            })
          }

          if (typeof pl[f] === "object") {
            pl[f] = module.exports.getAndChangeDateKeys(pl[f])
          }

          if (date_arr.includes(f)) {
            try {
              pl[f] = pl[f] != 0 ? new Date(pl[f]).toISOString() : pl[f]
            }
            catch (err) {
              // pl[f] = pl[f]
            }
          }
          if (del_key.includes(f)) {
            try {
              delete pl[f]
            }
            catch (err) { }
          ...
          //form html link
          if (pl["_links"] && pl["_links"]["webui"] && pl["_links"]["self"]) {
            pl["_links"]["htmlUrl"] = pl["_links"]["self"].split("/rest")[0] + "" + pl["_links"]["webui"]
          }
        }
